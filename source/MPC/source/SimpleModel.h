...
public:
	...
	bool createModel(){
		...
		model << dot(uHat) == delta_tDot;
		model << dot(vHat) == 0.0;
		model << dot(wHat) == 0.0;
		model << dot(psiHat) == delta_rDot;
	...
	model << dot(E) == uHat * (cos(thetaHat) * sin(psiHat)) + vHat * (sin(phiHat) * sin(thetaHat) * sin(psiHat) + cos(phiHat) * cos(psiHat)) + wHat* ( cos(phiHat) * sin(thetaHat) * sin(psiHat) - sin(phiHat) * cos(psiHat));
